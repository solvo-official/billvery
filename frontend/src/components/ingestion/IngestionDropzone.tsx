import { ArrowRight, Check, CircleSlash, FileImage, FileText, FileUp, LoaderCircle, TriangleAlert, X } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState, type DragEvent } from "react";
import { StatusBadge } from "@/components/audit/status";
import { Button } from "@/components/ui/button";
import { Tooltip } from "@/components/ui/tooltip";
import { useNow } from "@/hooks/use-now";
import { ACCEPTED_TYPES, api, toApiError } from "@/lib/api";
...
type Phase = "uploading" | "extracting" | "auditing" | "complete" | "failed" | "cancelled";
    ...
    (file: File) => {
      ...
      api
          ...
          onEvent: (event) => {
            ...
            if (event.event === "extracting") {
              setJobs((all) =>
                all.map((j) =>
                  j.id === id
                    ? { ...j, phase: "extracting", model: event.model, reused: event.reused ?? j.reused, extractingSince: j.extractingSince ?? Date.now() }
                    : j,
                ),
              );
            }
            if (event.event === "auditing") patch(id, { phase: "auditing" });
          },
        })
        .then(({ audit, replayed }) => {
          remove(pendingId);
        ...
        .catch((error: unknown) => {
          ...
          if (apiError.code === "aborted") patch(id, { phase: "cancelled", controller: undefined });
          else patch(id, { phase: "failed", error: apiError.message, controller: undefined });
        });
    },
    [merge, remove, organization, reviewer, maxUploadBytes],
  );

  // Cancel in-flight uploads if the console unmounts.
  const jobsRef = useRef(jobs);
  jobsRef.current = jobs;
  useEffect(() => () => jobsRef.current.forEach((job) => job.controller?.abort()), []);
...
function JobRow({ job, onInspect }: { job: UploadJob; onInspect: (id: string) => void }) {
  const running = job.phase === "uploading" || job.phase === "extracting" || job.phase === "auditing";
  const now = useNow(job.phase === "extracting" ? 1_000 : 60_000);
  const stepIndex = STEPS.findIndex((step) => step.phase === job.phase);
  const elapsed = job.extractingSince ? Math.max(0, Math.round((now - job.extractingSince) / 1000)) : 0;
  const width = job.phase === "uploading" ? 12 : job.phase === "auditing" ? 92 : 100;
    ...
    <li className="px-4 py-3">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {job.phase === "failed" ? (
            <p className="mt-1 flex items-start gap-1.5 text-[12px] text-rejected">
            ...
            <>
                ...
                {STEPS.map((step, index) => {
                  ...
                  return (
                    <li key={step.phase} className="flex items-center gap-1.5">
                      ...
                      <span className={cn("flex items-center gap-1 text-[11px]", active ? "text-accent" : done ? "text-ink-2" : "text-ink-3")}>
                        {active ? (
              ...
              <div className="relative mt-2 h-[3px] overflow-hidden rounded-full bg-line" role="progressbar" aria-label="Audit progress" aria-valuetext={PHASE_LABEL[job.phase]}>
                {job.phase === "extracting" ? (
                  // Gemini gives no progress signal: an honest indeterminate bar, not a made-up percentage.
                  <div className="absolute inset-y-0 w-1/3 animate-indeterminate rounded-full bg-accent" />
                ) : (
                  <div
                    className={cn("h-full rounded-full transition-[width] duration-500 ease-out", job.phase === "complete" ? "bg-ink-3" : "bg-accent")}
                    style={{ width: `${width}%` }}
                  />
                )}
              </div>
              <div className="mt-1.5 flex items-center justify-between gap-2">
                <p className="text-[12px] text-ink-2" aria-live="polite">
                  {job.phase === "extracting" && job.reused ? "Reusing the extraction of an identical file…" : PHASE_LABEL[job.phase]}
                  {job.phase === "extracting" && !job.reused ? <span className="figure ml-1.5 text-ink-3">{elapsed}s</span> : null}
                </p>
                {running ? (
                  <Button size="xs" variant="ghost" onClick={() => job.controller?.abort()} aria-label={`Cancel ${job.name}`}>
